...
class RequestBoxCubit extends Cubit<RequestBoxState> {
  ...
  void init(BuildContext context) {
    ...
    pagingAdapterRec.initPageRequestListener(
      context,
      pagingAdapterRec.pagingController,
    );
  ...
  Future<void> getRequestBoxesBase(
    ...
    required RequestBoxState Function(RequestBoxes? data, String? message)
        success,
    required RequestBoxState Function(NetworkExceptions? networkException)
        failure,
    ...
    final response =
        await repository.getRequestBoxes(page: pageKey, nameList: nameList);
    ...
    response.when(
      success: (data) async {
        ...
        if (requestBoxes?.listRequestBox.isEmpty ?? true) {
          ...
          emit(
            success(requestBoxes, data.message),
          );
        }
      },
      failure: (networkException) {
  ...
  Future<void> getRequestBoxesImp(BuildContext context, String nameList
    ...
    getRequestBoxesBase(context,
        loading: () => first
            ? const RequestBoxState.loadingFirst()
            : const RequestBoxState.loadingPagination(),
        empty: (message) => first
            ? RequestBoxState.emptyFirst(message)
            : RequestBoxState.emptyPagination(message),
        success: (data, message) => first
            ? RequestBoxState.successFirst(data, message)
            : RequestBoxState.successPagination(data, message),
        failure: (networkException) => first
            ? RequestBoxState.failureFirst(networkException)
  ...
  Future<void> getRequestBoxes(BuildContext context, String nameList
    ...
    response.when(
      success: (data) async {
        ...
        if (requestBoxes?.listRequestBox.isEmpty ?? true) {
          emit(
            RequestBoxState.emptyPagination(data.message),
          );
        } else {
          emit(
            RequestBoxState.successPagination(requestBoxes, data.message),
          );
        }
      },
      failure: (networkException) {
  ...
  Future<void> getRecRequestBoxes(BuildContext context,
    ...
    final response =
        await repository.getRequestBoxes(page: pageKey, nameList: 'recived');
    response.when(
      success: (data) async {
        List list = data.data.listRequestBox;
        recRequestsBox.addAll(list.whereType());
        pagingAdapterRec.handlePageData(
            baseModel: data,
            pageData: recRequestsBox,
            pageKey: pageKey,
            pagingController: pagingAdapterRec.pagingController);
        emit(
          RequestBoxState.successPagination(null, data.message),
        );
      },
  ...
  bool buildListSendBoxesPaginateWhen(
      RequestBoxState previous, RequestBoxState current) {
    if (current == previous) return false;
    return current.maybeWhen(
        loadingPagination: () => true,
  ...
  Widget buildListSendBoxesPaginate(
          BuildContext context, RequestBoxState state) =>
      state.maybeWhen(
        loadingPagination: () => const LoadingDataView(),
        failurePagination: (networkExceptions) =>
            ErrorView(networkExceptions: networkExceptions),
  ...
  Widget buildMailBoxPage(
          BuildContext context, RequestBoxState state, Widget child) =>
      state.maybeWhen(
          loadingPage: () => const LoadingDataView(),
          failurePage: (networkExceptions) =>
              ErrorView(networkExceptions: networkExceptions),
          emptyPagination: (message) => EmptyDataView(),
          orElse: () => child);
}
