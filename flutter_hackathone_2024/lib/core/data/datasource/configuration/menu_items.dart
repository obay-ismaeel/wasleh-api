...
class MenuItems {
  ...
  MenuItem? getOneMenuItem(MenuItem menuItem) {
    MenuItem? resultMenuItem = _menuItems
        .where((element) => element.title == menuItem.title)
        .map(
          (e) => MenuItem(
            title: e.title,
            route: menuItem.route,
            onTap: menuItem.onTap,
            icon: menuItem.icon ?? e.icon,
            queryParameters: menuItem.queryParameters,
            showDialog: menuItem.showDialog,
          ),
        )
        .toList()
        .firstOrNull;
